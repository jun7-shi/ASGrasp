...
class Combined_Volume:
    def __init__(self, raw_volume, att_volume, num_levels=2, radius=4):
        self.num_levels = num_levels
        self.radius = radius
        self.att_volume_pyramid = []
        self.corr_pyramid = []

        # all pairs correlation
        b, c, d, h, w = raw_volume.shape
        raw_volume = raw_volume.permute(0, 3, 4, 1, 2).reshape(b * h * w, 1, 1, d)
        att_volume = att_volume.permute(0, 3, 4, 1, 2).reshape(b * h * w, 1, 1, d)
        self.att_volume_pyramid.append(att_volume)
        self.corr_pyramid.append(raw_volume)
        for i in range(self.num_levels):
            att_volume = F.avg_pool2d(att_volume, [1, 2], stride=[1, 2])
            self.att_volume_pyramid.append(att_volume)

        for i in range(self.num_levels):
            raw_volume = F.avg_pool2d(raw_volume, [1, 2], stride=[1, 2])
            self.corr_pyramid.append(raw_volume)
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            corr = self.corr_pyramid[i]
            att_corr = self.att_volume_pyramid[i]
            ...
            corr = corr.view(batch, h1, w1, -1)
            att_corr= bilinear_sampler(att_corr, disp_lvl)
            att_corr = att_corr.view(batch, h1, w1, -1)
            out_pyramid.append(corr)
            out_pyramid.append(att_corr)
...
class CorrBlock1D_Cost_Volume:
    def __init__(self, corr, num_levels=2, radius=4, inverse_depth_min=None, inverse_depth_max=None, num_sample=None):
        self.num_levels = num_levels
        self.radius = radius
...
class CorrBlock1D_Cost_Volume_MD:
    def __init__(self, corr_fg, corr_bg, num_levels=2, radius=4, inverse_depth_min=None, inverse_depth_max=None, num_sample=None):
        self.num_levels = num_levels
        self.radius = radius
        self.inverse_depth_min = inverse_depth_min
        self.inverse_depth_max = inverse_depth_max
        self.num_sample = num_sample
        self.corr_pyramid_bg = []
        self.corr_pyramid_fg = []
        # all pairs correlation

        # batch, h1, w1, dim, w2 = corr.shape
        b, c, d, h, w = corr_fg.shape
        corr_fg = corr_fg.permute(0, 3, 4, 1, 2).reshape(b*h*w, 1, 1, d)
        corr_bg = corr_bg.permute(0, 3, 4, 1, 2).reshape(b*h*w, 1, 1, d)
        self.corr_pyramid_fg.append(corr_fg)
        self.corr_pyramid_bg.append(corr_bg)

        for i in range(self.num_levels):
            corr_fg = F.avg_pool2d(corr_fg, [1,2], stride=[1,2])
            corr_bg = F.avg_pool2d(corr_bg, [1, 2], stride=[1, 2])
            self.corr_pyramid_fg.append(corr_fg)
            self.corr_pyramid_bg.append(corr_bg)

    def __call__(self, coords_fg,  coords_bg):
        corr_fg = self.get(coords_fg, self.corr_pyramid_fg)
        corr_bg = self.get(coords_bg, self.corr_pyramid_bg)
        return corr_fg, corr_bg

    def get(self, coords, corr_pyramid):
        r = self.radius
