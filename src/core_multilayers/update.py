...
class FlowHeadClip(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256, output_dim=2):
        super(FlowHeadClip, self).__init__()
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        ...
        self.flow_head = FlowHeadClip(hidden_dims[2], hidden_dim=256, output_dim=1)
    ...
    def forward(self, net, inp, corr=None, flow=None, iter08=True, iter16=True, iter32=True, update=True):
        ...
        mask = self.mask(net[0])

        return net, mask, delta_flow

class BasicMultiUpdateBlock_2layer(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        super().__init__()
        self.args = args
        self.encoder = BasicMotionEncoder(args)
        self.encoder1 = BasicMotionEncoder(args)
        encoder_output_dim = 128

        self.gru08 = ConvGRU(hidden_dims[2], encoder_output_dim*2 + hidden_dims[1] * (args.n_gru_layers > 1))
        self.gru16 = ConvGRU(hidden_dims[1], hidden_dims[0] * (args.n_gru_layers == 3) + hidden_dims[2])
        self.gru32 = ConvGRU(hidden_dims[0], hidden_dims[1])
        # self.flow_head = FlowHead(hidden_dims[2], hidden_dim=256, output_dim=1)
        self.flow_head = FlowHeadClip(hidden_dims[2], hidden_dim=256, output_dim=2)
        factor = 2**self.args.n_downsample
    ...
    def forward(self, net, inp, corr=None, flow=None, iter08=True, iter16=True, iter32=True, update=True):
        ...
        if iter16:
            if self.args.n_gru_layers > 2:
                ...
                net[1] = self.gru16(net[1], *(inp[1]), pool2x(net[0]))
        if iter08:
            motion_features = self.encoder(flow[:,0:1,:,:], corr[:,0:36,:,:])
            motion_features1 = self.encoder1(flow[:,1:2,:,:], corr[:,36:72,:,:])
            motion_features = torch.cat([motion_features, motion_features1], axis=1)
            if self.args.n_gru_layers > 1:
                net[0] = self.gru08(net[0], *(inp[0]), motion_features, interp(net[1], net[0]))
            else:
                net[0] = self.gru08(net[0], *(inp[0]), motion_features)
