...
def readDispDreds(filename):
    depth = cv2.imread(filename, -1)
    depth = depth[:,:,0]
    f = 446.31
    baselineDis = 0.055
    disp = (f*baselineDis)/(depth+1e-6)
    valid = disp > 0.0
    return disp, valid

def readDepthDreds(filename):
    depth = cv2.imread(filename, -1)
    depth = depth[:,:,0]
    valid = (depth > 0.0) & (depth < 1.6)
    return depth, valid


def readDepthDredsPNG(filename):
    depth = cv2.imread(filename, -1).astype(np.float32)/1000.0
    return depth
